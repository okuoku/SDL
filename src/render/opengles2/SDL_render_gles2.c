...
GLES2_UpdateViewport(SDL_Renderer * renderer)
{
    ...
    if (renderer->target) {
        ...
        glViewport(renderer->viewport.x, (h - renderer->viewport.y - renderer->viewport.h),
                         renderer->viewport.w, renderer->viewport.h);
    }
...
GLES2_UpdateClipRect(SDL_Renderer * renderer)
{
    ...
    if (renderer->clipping_enabled) {
        ...
        if (renderer->target) {
            glScissor(renderer->viewport.x + rect->x, renderer->viewport.y + rect->y, rect->w, rect->h);
        } else {
            int w, h;

            SDL_GetRendererOutputSize(renderer, &w, &h);
            glScissor(renderer->viewport.x + rect->x, h - renderer->viewport.y - rect->y - rect->h, rect->w, rect->h);
        }
    } else {
    ...
    if (data) {
        ...
        if (data->context) {
            while (data->framebuffers) {
                ...
                glDeleteFramebuffers(1, &data->framebuffers->FBO);
...
GLES2_CreateTexture(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (data->yuv) {
        glGenTextures(1, &data->texture_v);
        if (GL_CheckError("glGenTexures()", renderer) < 0) {
            return -1;
        }
        glActiveTexture(GL_TEXTURE2);
        glBindTexture(data->texture_type, data->texture_v);
        ...
        glTexImage2D(data->texture_type, 0, format, texture->w / 2, texture->h / 2, 0, format, type, NULL);

        glGenTextures(1, &data->texture_u);
        if (GL_CheckError("glGenTexures()", renderer) < 0) {
            return -1;
        }
        glActiveTexture(GL_TEXTURE1);
        glBindTexture(data->texture_type, data->texture_u);
        glTexParameteri(data->texture_type, GL_TEXTURE_MIN_FILTER, scaleMode);
        glTexParameteri(data->texture_type, GL_TEXTURE_MAG_FILTER, scaleMode);
        glTexParameteri(data->texture_type, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(data->texture_type, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    if (data->nv12) {
        ...
        glTexParameteri(data->texture_type, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
        glTexParameteri(data->texture_type, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
        glTexImage2D(data->texture_type, 0, GL_LUMINANCE_ALPHA, texture->w / 2, texture->h / 2, 0, GL_LUMINANCE_ALPHA, GL_UNSIGNED_BYTE, NULL);
        if (GL_CheckError("glTexImage2D()", renderer) < 0) {
            return -1;
        }
    }
                    ...
                    const void *pixels, int pitch)
{
    ...
    if (tdata->yuv) {
        ...
        if (texture->format == SDL_PIXELFORMAT_YV12) {
            glBindTexture(tdata->texture_type, tdata->texture_v);
        } else {
            glBindTexture(tdata->texture_type, tdata->texture_u);
        }
        GLES2_TexSubImage2D(data, tdata->texture_type,
                rect->x / 2,
                rect->y / 2,
...
GLES2_SetRenderTarget(SDL_Renderer * renderer, SDL_Texture * texture)
{
    ...
    if (texture == NULL) {
        glBindFramebuffer(GL_FRAMEBUFFER, data->window_framebuffer);
    } else {
...
GLES2_DestroyTexture(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (tdata) {
        ...
        if (tdata->texture_v) {
            glDeleteTextures(1, &tdata->texture_v);
        }
        if (tdata->texture_u) {
            glDeleteTextures(1, &tdata->texture_u);
        }
                   ...
                   GLES2_ShaderCacheEntry *fragment, SDL_BlendMode blendMode)
{
    ...
    entry->uniform_locations[GLES2_UNIFORM_TEXTURE_V] =
        glGetUniformLocation(entry->id, "u_texture_v");
    entry->uniform_locations[GLES2_UNIFORM_TEXTURE_U] =
        glGetUniformLocation(entry->id, "u_texture_u");
    ...
    glUseProgram(entry->id);
    glUniform1i(entry->uniform_locations[GLES2_UNIFORM_TEXTURE_V], 2);  /* always texture unit 2. */
    glUniform1i(entry->uniform_locations[GLES2_UNIFORM_TEXTURE_U], 1);  /* always texture unit 1. */
    glUniform1i(entry->uniform_locations[GLES2_UNIFORM_TEXTURE], 0);  /* always texture unit 0. */
    glUniformMatrix4fv(entry->uniform_locations[GLES2_UNIFORM_PROJECTION], 1, GL_FALSE, (GLfloat *)entry->projection);
    glUniform4f(entry->uniform_locations[GLES2_UNIFORM_MODULATION], 1.0f, 1.0f, 1.0f, 1.0f);
    glUniform4f(entry->uniform_locations[GLES2_UNIFORM_COLOR], 1.0f, 1.0f, 1.0f, 1.0f);
    ...
    if (data->program_cache.count > GLES2_MAX_CACHED_PROGRAMS) {
        ...
        glDeleteProgram(data->program_cache.tail->id);
...
GLES2_RenderClear(SDL_Renderer * renderer)
{
    ...
    if (!CompareColors(data->clear_r, data->clear_g, data->clear_b, data->clear_a,
       ...
       if (renderer->target &&
        ...
        glClearColor((GLfloat) r * inv255f,
                     (GLfloat) g * inv255f,
                     (GLfloat) b * inv255f,
                     (GLfloat) a * inv255f);
...
GLES2_SetDrawingState(SDL_Renderer * renderer)
{
    ...
    if (!CompareColors(program->color_r, program->color_g, program->color_b, program->color_a, r, g, b, a)) {
        /* Select the color to draw with */
        glUniform4f(program->uniform_locations[GLES2_UNIFORM_COLOR], r * inv255f, g * inv255f, b * inv255f, a * inv255f);
        program->color_r = r;
        program->color_g = g;
        program->color_b = b;
        program->color_a = a;
                         ...
                         const void *vertexData, size_t dataSizeInBytes)
{
    ...
    if (!data->vertex_buffers[attr]) {
        glGenBuffers(1, &data->vertex_buffers[attr]);
    }

    glBindBuffer(GL_ARRAY_BUFFER, data->vertex_buffers[attr]);

    if (data->vertex_buffer_size[attr] < dataSizeInBytes) {
        glBufferData(GL_ARRAY_BUFFER, dataSizeInBytes, vertexData, GL_STREAM_DRAW);
        data->vertex_buffer_size[attr] = dataSizeInBytes;
    } else {
        glBufferSubData(GL_ARRAY_BUFFER, 0, dataSizeInBytes, vertexData);
    }

    glVertexAttribPointer(attr, attr == GLES2_ATTRIBUTE_ANGLE ? 1 : 2, GL_FLOAT, GL_FALSE, 0, 0);
#endif
...
GLES2_SetupCopy(SDL_Renderer *renderer, SDL_Texture *texture)
{
    ...
    if (!CompareColors(program->modulation_r, program->modulation_g, program->modulation_b, program->modulation_a, r, g, b, a)) {
        glUniform4f(program->uniform_locations[GLES2_UNIFORM_MODULATION], r * inv255f, g * inv255f, b * inv255f, a * inv255f);
...
static int GLES2_BindTexture (SDL_Renderer * renderer, SDL_Texture *texture, float *texw, float *texh)
{
    ...
    glBindTexture(texturedata->texture_type, texturedata->texture);
...
static int GLES2_UnbindTexture (SDL_Renderer * renderer, SDL_Texture *texture)
{
    ...
    glBindTexture(texturedata->texture_type, 0);
...
GLES2_ResetState(SDL_Renderer *renderer)
{
    ...
    glClearColor((GLfloat) data->clear_r * inv255f,
...
GLES2_CreateRenderer(SDL_Window *window, Uint32 flags)
{
...
#else /* !ZUNE_HD */
    glGetIntegerv(GL_SHADER_BINARY_FORMATS, (GLint *)data->shader_formats);
    if (hasCompiler) {
    ...
    glGetIntegerv(GL_FRAMEBUFFER_BINDING, &window_framebuffer);
